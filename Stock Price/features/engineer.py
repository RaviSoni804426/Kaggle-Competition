...
class FeatureEngineer:
    ...
    def create_labels(self, df: pd.DataFrame, horizon: int) -> pd.DataFrame:
        ...
        threshold = TARGET_THRESHOLDS.get(horizon, 3.0)
        ...
        df["future_return"] = future_return  # Keep for analysis
        
        logger.info(f"Labels created for {horizon}-day horizon (threshold: {threshold}%)")
        logger.info(f"Positive samples: {df['target'].sum()} / {df['target'].count()}")
